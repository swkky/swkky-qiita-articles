...
例えば、ユーザーが「月次の売上データを見せて」と聞いた際に、グロサリー用語 "月次売上" がテーブルにマッチする橋渡しをします。
...
| 2 | アセットの Description | 検索スコアリングに影響。 |
| 3 | Business Name（アセット＋カラム） | 整備コスト低（AI 生成可）。暗号的な名前には特に有効そう |
| 4 | グロサリー用語 (アセット+カラム) | 自然言語→テーブル変換に使用される。用語を事前に作成する必要がある |
