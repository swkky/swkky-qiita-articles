...
tags:
  ...
  - awscli
private: false
updated_at: '2026-08-28T16:33:19+09:00'
...
- [SageMaker Unified Studio 入門 〜アセットってなに？ どうやって作成するの？〜](https://qiita.com/swkky/items/092df5056ee13b7a9297)
...
- アセットが既に作成済みであること（[アセット作成手順](https://qiita.com/swkky/items/092df5056ee13b7a9297)を参照）
...
:::note
メタデータが付与されていない状態での実行はいいのですが、既存のメタデータが付与されている 2 回目以降、このあたりを考慮して実行するのは面倒なので、スクリプトを組んで、`get-asset`で既存フォーム、`get-form-type` で最新リビジョンを動的に取得して付与するのが良さそうです。
:::
...
| ビジネスコンテキスト | コマンド | 備考 |
|---------------------|---------|------|
| アセット Business Name | `create-asset-revision --name` | |
| アセット Description | `create-asset-revision --description` | max 2048文字 |
| アセット README | `create-asset-revision --forms-input` | `AssetCommonDetailsForm.readMe` |
| アセット グロサリー用語 | `create-asset-revision --glossary-terms` | max 20個 |
| アセット メタデータフォーム | `create-asset-revision --forms-input` | カスタムフォーム |
| カラム Business Name | `create-asset-revision --forms-input` | `ColumnBusinessMetadataForm.name` |
| カラム Description | `create-asset-revision --forms-input` | `ColumnBusinessMetadataForm.description` |
| カラム グロサリー用語 | `create-asset-revision --forms-input` | `ColumnBusinessMetadataForm.glossaryTerms` |
| カラム README | `batch-put-attributes-metadata` | `AttributeCommonFormType.readMe` |
| カラム メタデータフォーム | `batch-put-attributes-metadata` | カスタムフォーム |
