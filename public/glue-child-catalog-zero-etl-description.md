...
tags:
  ...
  - datacatalog
private: false
updated_at: '2026-08-25T17:05:14+09:00'
...
この名前だけでは **どの子カタログがどのデータソースに対応しているか判別できません。**
...
### 代替案 ：テーブルに対してアセットを作成して、Description を付与

子カタログ配下の**テーブル**に対してアセットを作成してカタログに公開すると、テーブルごとに説明や README を記載できます。
ただし、テーブル単位になるので、テーブルを大量に持つ RDS クラスターだとアセットも大量に必要になります。
アセットの作成方法については[こちらの記事](https://qiita.com/swkky/items/092df5056ee13b7a9297)で解説してます。
...
- 記載した説明はカタログ検索や SageMaker Data Agent の参照対象になる。
...
子カタログが削除できない以上、検証で作成した不要な子カタログが本番カタログに残ってしまいます。
AWS サポート経由で削除依頼は可能ですが、削除までにどれくらい掛かるか読めないので、**検証用と本番用でターゲットカタログを分けて運用することをおすすめします。**
...
| 制約 | 詳細 | 回避策 |
|------|------|--------|
| 子カタログに説明を付与できない | 「説明」フィールドへの書き込み手段なし | テーブルに対してアセット作成 -> アセットに対する Description などで補足 |
