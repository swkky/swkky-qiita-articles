---
title: SageMaker Unified Studio で Glue テーブルからアセットを作成する（AWS CLI 手順）
tags:
  - AWS
...
本記事は、AWS CLI を使って **Glue テーブルからアセットを作成する**具体的な手順（データソース作成 → データソースラン実行 → アセット確認）をまとめたハンズオン記事です。
